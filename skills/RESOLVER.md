...
| "duplicate person pages", "merge duplicate pages", "this person doesn't exist", "why are there two pages for", "clean up transcript names", "speaker names are wrong", "entity cleanup", "the name is misspelled in my brain" | `skills/transcript-identity-hygiene/SKILL.md` |
| "is my sync actually working", "audit the ingestion pipeline", "the cron says ok but nothing is showing up", "transcripts aren't reaching my brain", "why did the same meeting get ingested twice", "pipeline health check" | `skills/ingestion-pipeline-audit/SKILL.md` (run BEFORE transcript-identity-hygiene — don't clean while the pipeline still mints duplicates) |
...
## Book identifiers & cataloging

| Trigger | Skill |
|---------|-------|
| "isbn", "isbn-10 vs isbn-13", "check digit", "is this a valid isbn", "why did this isbn fail", "979 prefix", "sbn" | `skills/isbn/SKILL.md` |
| "does this need a new isbn", "new edition or same isbn", "set isbn", "box set isbn", "ebook isbn", "translation isbn", "trim size change" | `skills/isbn/SKILL.md` |
| "onix", "product relation code", "ean-13 barcode", "bisg title rule", "isli" | `skills/isbn/SKILL.md` |
| "this isbn looks wrong", "duplicate isbns", "concatenated isbn", "placeholder isbn", "one isbn on many titles" | `skills/isbn/SKILL.md` (dirty-data triage) |
| "marc record", "marc 21", "marc tag", "what does field 245 mean", "indicators", "subfield", "the leader", "008 field", "marcxml" | `skills/marc-records/SKILL.md` |
| "lccn", "oclc number", "authority control", "established heading", "lcsh", "sears", "catalog card to marc", "ils export" | `skills/marc-records/SKILL.md` |

> The two chain: `isbn` owns **the number**, `marc-records` owns **how the number is
> cataloged** (Field 020 `$a`/`$q`/`$z`). Read both when a question spans them. Both are
> mirrors of an upstream canonical copy and carry the standards only.
