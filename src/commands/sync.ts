...
async function stampSyncFreshness(engine: BrainEngine, sourceId: string | undefined): Promise<void> {
  if (sourceId) {
    await engine.executeRaw(
  ...
  await engine.setConfig('sync.last_sync_at', new Date().toISOString());
}
...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  if (lastCommit === headCommit && !versionMismatch && !versionNeverSet && !hasDetachedWorkingTreeChanges) {
    // A no-op sync is still a successful freshness proof. Previously this
    // returned before any durable stamp, so Doctor kept reporting unchanged
    // sources as stale even after `gbrain sync --source <id>` succeeded.
    await stampSyncFreshness(engine, opts.sourceId);
    return {
      status: 'up_to_date',
      fromCommit: lastCommit,
      toCommit: headCommit,
